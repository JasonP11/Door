...
void getuid() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    HTTPClient http;
    WiFiClient client;
    http.begin(client, serverUrl + "get_uids");
...
void handleNFCReader(PN532& nfc, String readerType) {
  ...
  if (nfc.readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, &uidLength, 200)) {
    ...
    if (uidMap.count(uidString)) {
      ...
      relayActive = true;
      Serial.println("✅ Match: " + uidString + " | Role: " + uidMap[uidString]);
    } 
      ...
      pixels.show();
      denyAccessBuzzer();
      Serial.println("❌ UID not found: " + uidString);
    }
  ...
  http.end();  // Move this inside the same block
