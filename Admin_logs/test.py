...
def fetch_logs():
    ...
    try:
        if date:
            cursor.execute("SELECT id, code, name, role, status, timestamp FROM access_logs WHERE DATE(timestamp) = %s ORDER BY id DESC", (date,))
            
        else:
            cursor.execute("SELECT id, code, name, role, status, timestamp FROM access_logs ORDER BY id DESC")
